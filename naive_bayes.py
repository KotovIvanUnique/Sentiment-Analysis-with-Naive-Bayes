...
class NaiveBayes:
    ...
    def _collect_word_stat(self):
        category_prob_sent = self._get_category_probability(self.categories[1]['total'])
        category_prob_emot = self._get_category_probability(self.categories[0]['total'])
        # Для всех слов
        for word in self.words.keys():
            # print(self._P_feat_class(1, word), Decimal(category_prob_spam))
            p1 = log(self._P_feat_class(1, word) * Decimal(category_prob_sent))
            p2 = log(self._P_feat_class(0, word) * Decimal(category_prob_emot))
